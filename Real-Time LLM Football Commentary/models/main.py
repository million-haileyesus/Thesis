...
def train_model(model, train_loader, validation_loader, epochs, optimizer_name, criterion, learning_rate, device, verbose=True, is_seq_model=False, is_gnn=False):
    ...
    for epoch in range(1, epochs + 1):
        train_loss, train_acc = train_one_epoch(model, train_loader, optimizer, criterion, device, is_seq_model=is_seq_model, is_gnn=is_gnn)
        val_loss, val_acc, val_metrics = validate_one_epoch(model, validation_loader, criterion, device, is_seq_model=is_seq_model, is_gnn=is_gnn)
